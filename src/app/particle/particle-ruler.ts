...
export class Ruler {
  ...
  public setSize(ws: number, hs: number): void {
    ...
    const graphics = this._shapeBg.graphics;
    graphics.clear();
    ...
    const distance  = 35;
    const w: number = Math.floor(this._width / distance);
    const h: number = Math.floor(this._height / distance);
  ...
  public update() {
    ...
    const mousePt  = this.container.globalToLocal(stage.mouseX, stage.mouseY);
    const graphics = this._shapeMouse.graphics;

    graphics.clear()
      .setStrokeStyle(2)
      .beginStroke('#6699ff');

    const isInHorizontal: boolean = 0 <= mousePt.x && mousePt.x <= this._width;
    const isInVertical: boolean   = 0 <= mousePt.y && mousePt.y <= this._height;
    ...
    if (this._isMouseDown === true) {
      ...
      const mx: number = Math.floor(mousePt.x) - 0.5;
      const my: number = Math.floor(mousePt.y) - 0.5;
